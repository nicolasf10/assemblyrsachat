...
sin_addr:	.dword 16777343	// inet_addr("127.0.0.1")
	.zero 8			// Not used
...
connect:
	...
	mov x8, #203		// Load syscall number 203 (connect) into x8
	svc #0			// Syscall	
	cmp x0, #0		
	beq server_connected_print // Error if bind doesn't return 0

	// Print error message and exit
	ldr x1, =error_connecting
	mov x1, #1		// stdout
	mov x2, #30
...
server_connected_print:
	// Print message "Server connected"
	ldr x1, =server_connected
	mov x0, #1		// stdout
	mov x2, #20
	mov x8, #64		// Syscall number for write
	svc #0			// Syscall
...
swap_keys:
	...
	mov w0, w4		// Save newsockfd for later
...
setup_poll:
	//brk #0
	// Setting up polling
	ldr x3, =sockfd	// Load address of newsockfd (client file descriptor) into x3
...
read_msg:
	...
	ldr x1, =sockfd	// Load newsockfd address into x1
	mov x0, #0
...
close_socket:
	ldr x1, =sockfd	// Load sockfd address
	mov x0, #0
